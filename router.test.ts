...
Deno.test('Matches routes', async () => {
  ...
  const user = await router(new Request(baseUrl + '/user/123', { 'method': 'GET' }));

  t.assertEquals(user.status, 200);
  t.assertEquals((await user.json()).id, '123');
  t.assertEquals((await router(new Request(baseUrl + '/user/123', { method: 'POST' }))).status, 200);
...
Deno.test('Return 404 on unknown route', async () => {
  ...
  t.assertEquals((await router(new Request(baseUrl + '/unknown', { method: 'POST' }))).status, 404);
});
